    ...
    const char  *mode)			// I - Open mode - "r" for reading or "w" for writing
{
  ...
  *nameptr = '\0';
  ...
  // Zephyr VFS does not support O_NOFOLLOW, O_CLOEXEC, or O_BINARY
  // Use only the standard POSIX flags supported by Zephyr's FS layer
  if (!strcmp(mode, "r"))
    return (open(fname, O_RDONLY));
  else if (!strcmp(mode, "w"))
    return (open(fname, O_WRONLY | O_CREAT | O_TRUNC, 0600));
    ...
    pappl_printer_t *printer)		// I - Printer
{
  ...
  for (job = (pappl_job_t *)cupsArrayGetFirst(printer->active_jobs); job; job = (pappl_job_t *)cupsArrayGetNext(printer->active_jobs))
  {
    ...
    {
#ifdef __ZEPHYR__
      // Zephyr: use k_thread_create to spawn job processing thread
      static K_THREAD_STACK_DEFINE(job_stack, CONFIG_PAPPL_JOB_STACK_SIZE);
      static struct k_thread job_thread_data;

      papplLogPrinter(printer, PAPPL_LOGLEVEL_DEBUG, "Starting job %d.", job->job_id);

      k_tid_t tid = k_thread_create(&job_thread_data, job_stack,
                                    K_THREAD_STACK_SIZEOF(job_stack),
                                    (k_thread_entry_t)_papplJobProcess,
                                    job, NULL, NULL,
