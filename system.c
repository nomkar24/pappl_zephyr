//
// System object for the Printer Application Framework (Zephyr Port)
//
    ...
    bool             tls_only)		// I - Only support TLS connections?
{
  ...
  // Disable raw socket options on Zephyr by default if they rely on Windows/POSIX specifics
  ...
  if (!system->name || !system->dns_sd_name || (subtypes && !system->subtypes))
    goto fatal;
  ...
  system->log_is_syslog = false;

  cupsMutexLock(&system->log_mutex);
  _papplLogOpenNoLock(system);
  cupsMutexUnlock(&system->log_mutex);

  // Initialize base filters (no ipptransform subprocesses on Zephyr)
