//
// Private system header file for the Printer Application Framework (Zephyr Port)
//
...
#define _PAPPL_MAX_LISTENERS 8  // Reduced from 32 to save RAM on ESP32/Zephyr
...
typedef struct _pappl_resource_s      // Resource
{
  char                  *path,          // Path
                        ...
                        *filename,      // Filename (unused/NULL on Zephyr)
                        *language;      // Language (for strings)
  time_t                last_modified;  // Last-Modified date/time
  const void            *data;          // Static in-memory data (e.g. en_strings)
...
struct _pappl_system_s                  // System data
{
  ...
  char                  *domain_path;   // Domain socket path (unused/NULL on Zephyr)
  cups_array_t          *host_aliases;  // Host aliases (unused/NULL on Zephyr)
  ...
  char                  *directory;     // Spool directory (unused/NULL on Zephyr)
  cups_mutex_t          log_mutex;      // Log mutex
  char                  *log_file;      // Log filename (unused/NULL on Zephyr)
  int                   log_fd;         // Log file descriptor (unused on Zephyr)
  pappl_loglevel_t      log_level;      // Log level
  size_t                log_max_size;   // Maximum log file size (unused on Zephyr)
  bool                  log_is_syslog;  // Log to system log? (always false on Zephyr)
  char                  *subtypes;      // DNS-SD sub-types, if any
  bool                  tls_only;       // Only support TLS?
  int                   idle_shutdown;  // Idle shutdown limit in seconds
  char                  *auth_service;  // PAM authorization service (unused on Zephyr)
  char                  *admin_group;   // PAM administrative group (unused on Zephyr)
  gid_t                 admin_gid;      // PAM administrative group ID (unused on Zephyr)
  char                  *default_print_group; // Default PAM printing group (unused on Zephyr)
  ...
  cups_array_t          *localizations; // Array of localizations (English-only to save RAM)
  ...
  char                  *ipptransform;  // Path to ipptransform command (unused/NULL on Zephyr)

  cups_mutex_t          ext_mutex;      // Mutex for commands
  int                   ext_next_number; // Next command number
  cups_array_t          *ext_commands,  // Commands (unused on Zephyr)
                        *ext_readexec,  // Read/execute paths (unused on Zephyr)
                        *ext_readonly,  // Read-only paths (unused on Zephyr)
                        *ext_readwrite; // Read/write paths (unused on Zephyr)
  char                  *ext_user,      // User for external commands (unused on Zephyr)
                        *ext_group;     // Group for external commands (unused on Zephyr)
};
...
extern bool             _papplSystemWebAddPrinter(pappl_client_t *client, pappl_system_t *system) _PAPPL_PRIVATE;
extern bool             _papplSystemWebConfig(pappl_client_t *client, pappl_system_t *system) _PAPPL_PRIVATE;
extern void             _papplSystemWebConfigFinalize(pappl_system_t *system, size_t num_form, cups_option_t *form) _PAPPL_PRIVATE;
extern bool             _papplSystemWebHome(pappl_client_t *client, pappl_system_t *system) _PAPPL_PRIVATE;
...
extern bool             _papplSystemWebNetwork(pappl_client_t *client, pappl_system_t *system) _PAPPL_PRIVATE;
extern bool             _papplSystemWebSecurity(pappl_client_t *client, pappl_system_t *system) _PAPPL_PRIVATE;
...
extern bool             _papplSystemWebWiFi(pappl_client_t *client, pappl_system_t *system) _PAPPL_PRIVATE;
